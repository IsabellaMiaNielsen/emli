...
// Configuration
#define WIFI_SSID       "EMLI-TEAM-7"
#define WIFI_PASSWORD    "group7group7"

#define MQTT_SERVER      "192.168.10.1"
#define MQTT_SERVERPORT  1883 
#define MQTT_USERNAME    "group7"
#define MQTT_KEY         "Group7"
...
void publish_data(int buttonState)
{
  char payload[10];
  ...
  if((WiFiMulti.run(conn_tout_ms) == WL_CONNECTED))
  {
    ...
    mqtt_connect();
    if (! count_mqtt_publish.publish(buttonState))
    {
      debug("MQTT failed");
    }
    else
...
void loop()
{
    if (millis() - prev_post_time >= PUBLISH_INTERVAL)
    {
      ...
      int buttonState = digitalRead(GPIO_INTERRUPT_PIN);
